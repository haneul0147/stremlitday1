...
def main():
    ...
    my_choics=st.selectbox('좋아하는 언어를 선택하세요',language)
    if my_choics == 'c':
        st.write('저는 C가 좋아요')
    elif my_choics == 'python':
        st.write('파이썬이 최고다')

    choice_list=st.multiselect('여러개를 선택할 수 있습니다.',language)
    # 여러분들이 디버깅을 하고싶으면,
    # 파이선의 print 함수를 이용하면,아래의 터미널에
    # 출력이 된다. 
    print(choice_list)
    ...
    age=st.slider('나이',1,100,value=50)    
    st.write('선택한 나이는 {}세 입니다'.format(age))
